...
def test_meerdere_tijdsblokken_blijven_apart():
    # ochtend + namiddag -> twee blokken; de middagpauze blijft zichtbaar
    r = parse_osm_uren("Mo-Fr 09:00-12:00,13:00-18:00")
    assert r["ma"] == [["09:00", "12:00"], ["13:00", "18:00"]]
...
def test_sync_respecteert_bestaande_uren(app):
    ...
    with app.app_context():
        ...
        # 2) bron levert bij hersync betere uren -> die verversen wel
        data["openingsuren"] = {"ma": [["09:00", "12:00"], ["13:00", "17:00"]]}
        upsert_event(data)
        db.session.commit()
        db.session.refresh(ev)
        assert ev.openingsuren["ma"] == [["09:00", "12:00"], ["13:00", "17:00"]]
        # 3) beheerder stelt handmatig in (formulier zet de marker) -> sync blijft eraf
        ev.openingsuren = {"ma": [["08:00", "22:00"]], "zo": None, "_handmatig": True}
        db.session.commit()
        data["openingsuren"] = {"ma": [["10:00", "16:00"]]}
        upsert_event(data)
        db.session.commit()
        db.session.refresh(ev)
        assert ev.openingsuren["ma"] == [["08:00", "22:00"]]
